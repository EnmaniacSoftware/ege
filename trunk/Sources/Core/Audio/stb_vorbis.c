...
#define SETUP_MALLOC(f, size) setup_malloc((f), (size))
//#define SETUP_MALLOC(f, size) setup_malloc((f), (size), __FUNCTION__, __LINE__)
...
int stb_vorbis_decode_filename(char *filename, int *channels, short **output)
{
   ...
   *channels = v->channels;
   offset = data_len = 0;
   total = limit;
   data = (short *) EGE_MALLOC(total * sizeof(*data));
   if (data == NULL) {
      stb_vorbis_close(v);
      return -2;
   }

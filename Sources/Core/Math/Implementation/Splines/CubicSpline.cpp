...
bool CubicSpline::addPoints(const List<Vector3f>& points)
{
  ...
  if ((m_segments.isEmpty() && ((KFirstSegmentPointCount == points.size()) || (0 == (points.size() - KFirstSegmentPointCount) % KNextSegmentPointCount))) ||
      ( ! m_segments.isEmpty() && (0 == (points.size() % KNextSegmentPointCount))))
  {

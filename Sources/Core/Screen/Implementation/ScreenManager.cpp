...
void ScreenManager::update(const Time& time)
{
  if (!m_screens.isEmpty())
  {
    ...
    for (ScreenList::ReverseIterator it = localCopy.rbegin(); it != localCopy.rend(); ++it)
    {
      PScreen& screen = *it;      
      if (screen->isEnabled())
      {
...
void ScreenManager::show(PScreen screen)
{
  ...
  {
    ScreenList::ReverseIterator it  = m_screens.rbegin();
    ++it;
    ++it;
    ScreenList::ReverseIterator end = m_screens.rend();
...
void ScreenManager::render(Viewport* viewport, IRenderer* renderer)
{
  ...
  for (ScreenList::ConstReverseIterator it = m_screens.rbegin(); it != m_screens.rend(); ++it)
  {
...
void ScreenManager::notifyPointerEvent(const PointerEvent& event)
{
  ...
  if ( ! m_screens.isEmpty())
  {
    ...
    for (ScreenList::ReverseIterator it = m_screens.rbegin(); it != m_screens.rend(); ++it)
    {
      PScreen& screen = *it;      
      if (screen->isEnabled())
      {
...
{
  // go thru all screens
  for (ScreenList::ConstIterator it = m_screens.begin(); it != m_screens.end(); ++it)
  {
    // check if found
    if ((*it)->name() == name)
    {
